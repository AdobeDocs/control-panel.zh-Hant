...
+ 探索「控制面板」{#discover-control-panel}
   ...
   + [管理對控制面板的權限](discover/using/managing-permissions.md)
...
+ SFTP 管理{#sftp-management}
   ...
   + [管理SFTP儲存](sftp/using/sftp-storage-management.md)
   + [IP 範圍允許清單](sftp/using/ip-range-allow-listing.md)
   + [金鑰管理](sftp/using/key-management.md)
   + [登錄到SFTP伺服器](sftp/using/logging-into-sftp-server.md)
...
+ 子網域和憑證{#subdomains-and-certificates}
   ...
   + 續訂SSL證書 {#renew-ssl}
      + [關於SSL證書續訂](subdomains-certificates/using/renewing-subdomain-certificate.md)
      + [生成CSR](subdomains-certificates/using/generate-csr.md)
      + [安裝SSL證書](subdomains-certificates/using/install-ssl-certificate.md)
+ 效能監視{#performance-monitoring}
   ...
   + 資料庫監視 {#database-monitoring}
      ...
      + [防止資料庫過載](performance-monitoring/using/database-preventing-overload.md)
