...
一種方式是導覽至[「Adobe Experience Cloud 首頁](https://experiencecloud.adobe.com/) >**[!UICONTROL Administration]**」。您會在「管理」底部找到組織ID **[!UICONTROL Quick Access]** 區段。 您可在 [Adobe Experience Cloud 文件](https://experienceleague.adobe.com/docs/core-services/interface/administration/organizations.html)中找到更多詳細資訊。

另一種方式是啟動 **Admin Console**。您的組織ID將會顯示在您的URL中，看起來會像這樣：https://adminconsole.adobe.com/xxx@AdobeOrg/overview。
...
如果您有多個Adobe解決方案的存取權，您可能會有多個組織ID。 在此情況下，您在Adobe Campaign例項下看到的組織ID就是您應使用的正確組織ID。

>[!NOTE]
>
>如果您的Adobe Campaign和Adobe Analytics有相同的組織ID，這是最理想的選擇。 如果您打算整合解決方案，以善用購物車放棄（針對AA + AC）等複雜的使用案例，Analytics和Campaign之間便需要有一個組織ID。
>
>如果您的Adobe Campaign和Adobe Analytics有不同的組織ID，請聯絡客戶服務以進行整合。
