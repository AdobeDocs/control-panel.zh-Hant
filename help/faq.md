...
如果您打算整合解決方案，以善用購物車放棄 (針對 Adobe Analytics + Adobe Campaign) 等複雜使用案例，Analytics 和 Campaign 之間便需要共用同一個組織 ID。如果您擁有多個 Adobe 解決方案的存取權限，您可能會有超過一個組織 ID。在此情況下，您在 Adobe Campaign 執行個體下看到的組織 ID 就是您應使用的正確 ID。
...
1. 擷取您的登入 URL。這是您用來登入 Campaign 執行個體的 URL，通常以「.campaign.adobe.com」或「.neolane.net」結尾。
