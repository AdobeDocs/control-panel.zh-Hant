...
若要有效監控您的 Campaign 執行個體，必須持續追蹤可能影響執行個體的重要事件。 通過「控制面板」，您可以識別新版本、升級、修補程式、熱修復程式等事件，並提供任何請求或問題的關鍵Adobe聯繫人清單。
...
* **提醒**&#x200B;由使用者設定，以便在事件發生之前收到通知。 日曆視圖中的這些顯示為綠色。[瞭解如何設定提醒](#reminders)
* **警示**&#x200B;由「控制面板」透過電子郵件傳送，通知使用者其執行個體出現的問題，例如儲存超過負荷或 SSL 憑證過期。 會在行事曆檢視以橘色顯示。 事件說明指定是否將警報發送給登錄用戶，具體取決於用戶對電子郵件警報的訂閱。[瞭解有關控制面板電子郵件警報功能的詳細資訊](../performance-monitoring/using/email-alerting.md)
...
>[!NOTE]
>
>為了獲得有關即將發生的事件的通知，請確保您已在「控制面板」中訂閱了電子郵件警報。[瞭解詳細資訊](../performance-monitoring/using/email-alerting.md)
