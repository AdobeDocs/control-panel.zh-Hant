...
>[!NOTE]
>
>Adobe 受管理 SSL 是免費提供給使用者的功能。 將子域的證書委託給Adobe是透明的，不會影響您的市場活動和交付能力。[瞭解有關SSL證書管理的詳細資訊](monitoring-ssl-certificates.md#management)
...
* **完整子網域委派**：Adobe 會自動要求安裝 SSL 憑證，您不用進行任何操作。 提交子域配置後，將立即作為子域設定工作流的一部分處理證書安裝請求。[瞭解有關完整子域委派的詳細資訊](setting-up-new-subdomain.md#full-subdomain-delegation)

* **CNAME 委派**：稍後會在設定精靈中，提供想複製到託管解決方案的憑證記錄。 在提交子域配置之前，您需要在域托管解決方案中生成這些證書記錄。[瞭解有關CNAME委派的詳細資訊](setting-up-new-subdomain.md#use-cnames)
